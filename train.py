...
# def compute_iou_loss(pred, target, num_classes=3):
#     iou_per_class = compute_iou(pred, target, num_classes)
#     iou_loss = 0
...
def parser():
    ...
    parser.add_argument('--savepath', default="/home/new-cod-sod-dataset/train/weight", type=str)
    # parser.add_argument('--datapath', default="/home/new-cod-sod-dataset/train", type=str)
...
def train(Network):
    ...
    for name, param in net.named_parameters():
        ...
        # else:
        #     tuning_slide.append(param)  # 1e-4
        tuning_slide.append(param)  # 1e-4
    ...
    for epoch in range(cfg.epoch):
        ...
        for step, sample in enumerate(train_loader):
            ...
            if target.max() > 2 or target.min() < 0:
                continue
            # # weight
            # weight = 1 / (torch.bincount(target.long().view(-1)).float() + 1e-6)
            # weight = weight / weight.sum()
            # # print("weight", weight)#weight tensor([0.0299, 0.2730, 0.6971], device='cuda:0')
            # criterion = nn.CrossEntropyLoss(weight=weight)

            coarse_map, Background_outputs, sod_outputs, cod_outputs = net(image, multimask_output=False, image_size=352)
            ...
            # criterion = nn.CrossEntropyLoss()
            coarse_map = torch.nn.functional.interpolate(coarse_map, size=(target.shape[1], target.shape[2]),
                                                         mode='nearest')  #
            ...
            # loss_iou = compute_iou_loss(pred, target)
            # loss = (loss_CE + loss_CE_coarse + loss_iou) / 3
            ...
            sw.add_scalars('loss_CE_coarse', {'loss_CE_coarse': loss_CE_coarse.item()}, global_step=global_step)


            if step % 80 == 0:
                print('%s | step:%d/%d/%d | loss=%.6f|lossCE=%.6f|' % (
                    datetime.datetime.now(), global_step, epoch + 1, cfg.epoch, loss.item(), loss.item()))

            torch.save(net.state_dict(), cfg.savepath + '/model-' + str(step + 1))
